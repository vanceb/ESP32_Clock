...
/* Function to handle display mode request command */
void changeDisplayMode ( kv *root )
{
    char *value = getValue ( root, "mode" );
    ESP_LOGI ( TAG, "Setting clock display mode to %s", value );
    request_display_mode = atoi ( value );
}


/* Function to handle firmware upgrade command */
int upgradeFirmware ( kv *root )
{
    ota_error_t err;
...
/* Parse incoming mqtt message for a command */
void parse_command ( char *buffer ) 
{
    ...
    if ( ( command != NULL ) && ( *command != '\0' ) ) {
        ...
        if ( strcmp ( command, "mode" ) == 0 ) {
            changeDisplayMode ( root );
        }
    
        /* Check for firmware upgrade */
        if ( strcmp ( command, "upgrade" ) == 0) {
            upgradeFirmware ( root );
        }
...
void sendQueuedTelemetry(esp_mqtt_client_handle_t client)
{
    ...
    if ( xQueueReceive ( telemetry_tx_queue, & ( telemetry_message ), 1000 / portTICK_PERIOD_MS ) ) {
        ...
        esp_mqtt_client_publish(client, telemetry_message->topic, telemetry_message->message, 0, 0, 0); 
